...
class Tokenizer():
    ...
    def encode(self, inputs):
        ...
        encoded_inputs['input_ids'] = torch.tensor(input_ids_padded)
        encoded_inputs['attention_mask'] = torch.tensor(attention_mask_padded)
...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        output0_config = pb_utils.get_output_config_by_name(
            model_config, "ENCODED_INPUT_TOKENS")

        # Convert Triton types to numpy types
        self.output0_dtype = pb_utils.triton_string_to_numpy(
            output0_config['data_type'])
        
        self.buffer = [RequestBuffer() for _ in range(self.max_batch_size)]
    ...
    def execute(self, requests):
            ...
            for idx, request in requests:
                # Get INPUT0
                in_0 = pb_utils.get_input_tensor_by_name(request, "INPUT_TOKENS")
                ...
                corr_id =(pb_utils.get_input_tensor_by_Name(request, "CORR_ID").as_numpy()==1)

                cid = corr_id[0,0]
                if is_seq_start[0,0]:
                    #initialize buffer
                    self.buffer[idx].reset(cid)
                elif self.buffer[idx].cid != cid:
                    self.buffer[idx].reset(cid)
                self.buffer[idx].string += 
                ...
                out_tensor_0 = pb_utils.Tensor("ENCODED_INPUT_TOKENS",
